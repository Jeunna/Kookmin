...
void Put_R2(Lock *L2, char *R2, CondVar *C2) {
  Acquire(L2);
  Store(R2,1);
  Signal(C2);
  // print message: getpid()가 R2을 내려놓음
  printf("%d put R2\n", getpid());
  Release(L2);
}

void Put_R3(Lock *L3, char *R3, CondVar *C3) {
  Acquire(L3);
  Store(R3,1);
  Signal(C3);
  // print message: getpid()가 R3을 내려놓음
  printf("%d put R3\n", getpid());
  Release(L3);
}

void Phil_A(Lock *L1, char *R1, CondVar *C1, Lock *L3, char *R3, CondVar *C3) 
{
  Take_R1(L1, R1, C1);
...
void Phil_B(Lock *L1, char *R1, CondVar *C1, Lock *L2, char *R2, CondVar *C2) 
{
  Take_R1(L1, R1, C1);
...
void Phil_C(Lock *L2, char *R2, CondVar *C2, Lock *L3, char *R3, CondVar *C3) 
{
  Take_R2(L2, R2, C2);
