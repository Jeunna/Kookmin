...
public class Inventory : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(this.gameObject.tag == "MainInventory")
        {
            ...
            {
                int itemValue = itemDatabase.getItemValueByID(itemDatabase.itemList[i].itemID);
                if (itemValue != 0)
                {
                    addItemToInventory(itemDatabase.itemList[i].itemID);
                }
            }
        }
        ...
        for (int i = 0; i < ItemsInInventory.Count; i++)
        {
            if (ItemsInInventory[i].itemID == itemID)
            {
                ...
                if (stack <= ItemsInInventory[i].maxStack)
                {
                    ...
                    itemDatabase.setItemValue(itemID, stack);
    ...
    public void addItemToInventory(int id)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount == 0)
            {
                ...
                itemDatabase.setItemValue(id, 1);
                break;
            }
        }
    ...
    public GameObject addItemToInventory(int id, int value)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount == 0)
            {
                ...
                itemDatabase.setItemValue(id, itemOnObject.item.itemValue);
                item.transform.SetParent(SlotContainer.transform.GetChild(i));
                item.GetComponent<RectTransform>().localPosition = Vector3.zero;
        ...
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount == 0)
            {
                ...
                if (itemOnObject.item.itemValue < itemOnObject.item.maxStack && value <= itemOnObject.item.maxStack)
                    ...
                    itemOnObject.item.itemValue = 1;
                itemDatabase.setItemValue(itemID, itemOnObject.item.itemValue);
                item.transform.SetParent(SlotContainer.transform.GetChild(i));
                item.GetComponent<RectTransform>().localPosition = Vector3.zero;
